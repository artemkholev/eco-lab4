# Инструкция по сборке и запуску EcoOS в QEMU
...
После запуска откроется графическое окно QEMU, где вы увидите:
- Регистрацию всех задач (синим цветом)
- Выполнение каждой задачи по приоритету (от наибольшей duration к наименьшей):
  - Сообщение о начале задачи (фиолетовым)
  - Прогресс-бар с процентами выполнения (зелёным)
  - Текущее время выполнения задачи (красным, в верхнем углу)
  - Сообщение о завершении задачи (зелёным)

## Детали реализации

### Планировщик задач

Планировщик выбирает задачу с наибольшим значением `duration` (приоритет) и выполняет её до конца. После завершения задача удаляется из списка и выбирается следующая задача с наибольшим приоритетом.
...
Каждая задача выводит:
- **Общее время выполнения** - в формате "Total duration: XXXXXXXms" (красным в верхнем левом углу)
- **Название текущей задачи** - в формате "In process: [Название]" (жёлтым, вверху)
- **Сообщение о регистрации** - перед выполнением (синим)
- **Сообщение о начале** - при старте задачи (фиолетовым)
- **Прогресс-бар** - визуальный индикатор `[========>        ] XX%` (зелёным)
- **Сообщение о завершении** - после окончания задачи (зелёным)
